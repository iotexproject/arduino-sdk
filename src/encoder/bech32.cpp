...
Encoding VerifyChecksum(const std::string& hrp, const data& values)
{
	...
	// instead. In Bech32m, this constant was amended.
...
data CreateChecksum(Encoding encoding, const std::string& hrp, const data& values)
{
	...
	uint32_t mod =
		PolyMod(enc) ^ EncodingConstant(encoding); // Determine what to XOR into those 6 zeroes.
	data ret(6);
...
{
	// First ensure that the HRP is all lowercase. BIP-173 and BIP350 require an
	// encoder to return a lowercase Bech32/Bech32m string, but if given an
	// uppercase HRP, the result will always be invalid.
